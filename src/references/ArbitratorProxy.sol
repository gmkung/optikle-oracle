 ...
 */
contract RealitioForeignArbitrationProxyWithAppeals is
    ...
    function fundAppeal(
        ...
        uint256 multiplier;
        {
            ...
            if (winner == _answer) {
                ...
                require(
                    block.timestamp - appealPeriodStart <
                        (appealPeriodEnd - appealPeriodStart).mulCap(
        ...
        uint256 contribution = totalCost.subCap(round.paidFees[_answer]) >
            msg.value
            ? msg.value
        ...
        if (msg.value.subCap(contribution) > 0)
            payable(msg.sender).send(msg.value.subCap(contribution)); // Sending extra value back to contributor. It is the user's responsibility to accept ETH.
